...
impl<'a, 'b> FifoReader<'a, 'b> {
    ...
    pub async fn read_fifo_results(&mut self) -> Result<(), RapLibErrors> {
        let result = self.request_asymmetry_results().await?;
        self.send_result(result).await?;

        let result = self.request_monobit_results().await?;
        self.send_result(result).await?;

        let result = self.request_runs_results().await?;
        self.send_result(result).await?;

        let result = self.request_sha256_results().await?;
        self.send_result(result).await?;

        Ok(())
    }

    async fn send_result<T: Into<DataType>>(&self, result: T) -> Result<(), RapLibErrors> {
    ...
    async fn request_asymmetry_results(&mut self) -> Result<Vec<i32>, RapLibErrors> {
        let mut asym_buffer: Vec<i32> = Vec::new();
        sanity_checks::req_read_asym_fifo(self.board)?;

        while let Ok(value_read) = timeout_with_err(&mut || self.board.read_32_bit_u32()).await? {
    ...
    async fn request_monobit_results(&mut self) -> Result<Vec<(f32, u32, u32)>, RapLibErrors> {
    ...
    async fn request_runs_results(&mut self) -> Result<Vec<(f64, u32, u32)>, RapLibErrors> {
        let mut runs_buffer: Vec<(f64, u32, u32)> = Vec::new();
        sanity_checks::req_read_runs_fifo(self.board)?;

        while let Ok(value_read) = timeout_with_err(&mut || self.board.read_64_bit_u64()).await? {
    ...
    async fn request_sha256_results(&mut self) -> Result<Vec<u8>, RapLibErrors> {
...
async fn timeout_with_err<F, T>(f: &mut F) -> Result<T, RapLibErrors>
where
    F: FnMut() -> T + Send,
{
    /*
